...
public class AppExceptionHandler {
    ...
    public ResponseEntity<?> capturerException(MethodArgumentNotValidException e, Locale locale) {
       ...
       String message = e.getFieldErrors().stream().map(DefaultMessageSourceResolvable::getDefaultMessage).collect(Collectors.joining(" / "));
        return ResponseEntity.status(HttpStatus.NOT_ACCEPTABLE).body(titre + "\n" + message);
